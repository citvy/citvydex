...
import { PrivateKey, PublicKey, Login, Aes } from "citvyjs"

class Citvy {
  static node = "wss://wallet.citvy.com/ws"
  ...
  static async connect(node, autoreconnect = Citvy.autoreconnect) {
    if (Citvy.connectPromise || Citvy.connectedPromise)
      return Promise.all([Citvy.connectPromise, Citvy.connectedPromise]);

    if (autoreconnect)
      Api.getApis().setRpcConnectionStatusCallback(Citvy.statusCallBack)

    await (Citvy.connectPromise = Citvy.reconnect(node));
    await (Citvy.connectedPromise = Citvy.connectedInit());
  ...
  static disconnect() {
    Citvy.connectPromise = Citvy.connectedPromise = undefined
    Citvy.autoreconnect = false
    Api.getApis().close()
  }

  static async reconnect(node = Citvy.node) {
    let res = await Api.getApis().instance(node, true).init_promise;
    Citvy.chain = res[0].network;
    Citvy.node = node
  ...
  static statusCallBack(status) {
    Citvy.logger.log("WebSocket status:", status)
    if (Citvy.autoreconnect && status === 'closed') {
      Citvy.logger.log("WebSocket status, try to connect...");
      setTimeout(() => {
        Citvy.reconnect().then(Event.resubscribe.bind(Event)).catch(Citvy.logger.error)
      }, 2000)
    }
  }
  ...
  static async login(accountName, password, feeSymbol = Citvy.chain.core_asset) {
    let
      acc = await Citvy.accounts[accountName],
    ...
    let account = new Citvy(accountName, activeKey.toWif(), feeSymbol);
  ...
  static async loginFromFile(buffer, password, accountName, feeSymbol = Citvy.chain.core_asset) {
    ...
    let acc = await Citvy.accounts[accountName];
  ...
  static ticker(baseSymbol, quoteSymbol) {
    return Citvy.db.get_ticker(baseSymbol.toUpperCase(), quoteSymbol.toUpperCase())
  }

  static async tradeHistory(quoteSymbol, baseSymbol, startDate, stopDate, bucketSeconds) {
    return Citvy.history.get_market_history(
      (await Citvy.assets[quoteSymbol]).id,
      (await Citvy.assets[baseSymbol]).id,
      bucketSeconds,
  ...
  static async getLimitOrders(quoteSymbol, baseSymbol, limit = 50) {
    return Citvy.db.get_limit_orders(
      (await Citvy.assets[quoteSymbol]).id,
      (await Citvy.assets[baseSymbol]).id,
      limit > 100 ? 100 : limit
    )
  ...
  static async getOrderBook(quoteSymbol, baseSymbol, limit = 50) {
    return Citvy.db.get_order_book(
      (await Citvy.assets[quoteSymbol]).id,
      (await Citvy.assets[baseSymbol]).id,
      limit > 50 ? 50 : limit
    )
  ...
  constructor(accountName, activeKey, feeSymbol = Citvy.chain.core_asset) {
    if (activeKey)
    ...
    this.initPromise = Promise.all([
      Citvy.accounts[accountName],
      Citvy.assets[feeSymbol]
    ]).then(params => {
  ...
  setFeeAsset = async feeSymbol => {
    ...
    this.feeAsset = await Citvy.assets[feeSymbol]
  }
  ...
  balances = async (...args) => {
    ...
    let assets = await Promise.all(args
      .map(async asset => (await Citvy.assets[asset]).id));
    let balances = await Citvy.db.get_account_balances(this.account.id, assets);
    return Promise.all(balances.map(balance => Citvy.assets.fromParam(balance)))
  }

  buyOperation = async (buySymbol, baseSymbol, amount, price, fill_or_kill = false, expire = "2020-02-02T02:02:02") => {
    await this.initPromise;

    let buyAsset = await Citvy.assets[buySymbol],
        baseAsset = await Citvy.assets[baseSymbol],
  ...
  buy = async (...args) => {
    ...
    return (await Citvy.db.get_objects([tx[0].trx.operation_results[0][1]]))[0];
  }

  sellOperation = async (sellSymbol, baseSymbol, amount, price, fill_or_kill = false, expire = "2020-02-02T02:02:02") => {
    await this.initPromise;

    let sellAsset = await Citvy.assets[sellSymbol],
        baseAsset = await Citvy.assets[baseSymbol],
  ...
  orders = async () => {
    await this.initPromise;
    return (await Citvy.db.get_full_accounts([this.account.id],false))[0][1].limit_orders
  }

  getOrder = async id => {
    await this.initPromise;
    return (await Citvy.db.get_objects([id]))[0];
  }
  ...
  memo = async (toName, message) => {
    ...
    let nonce = Date.now().toString(), //TransactionHelper.unique_nonce_uint64(),
        to = (await Citvy.accounts[toName]).options.memo_key;
  ...
  transferOperation = async (toName, assetSymbol, amount, memo) => {
    ...
    let asset = await Citvy.assets[assetSymbol],
    ...
    let params = {
      ...
      to: (await Citvy.accounts[toName]).id,
  ...
  assetIssueOperation = async (toName, assetSymbol, amount, memo) => {
    ...
    let params = {
      ...
      issue_to_account: (await Citvy.accounts[toName]).id
    };
